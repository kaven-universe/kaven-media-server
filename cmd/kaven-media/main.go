...
import (
	...
	"kaven.xyz/kaven/kaven-media-server/internal/applog"
...
func run() error {
	...
	switch command {
	case "serve":
		defer imageproc.ShutdownDefaultRuntime()
		...
		logger, logWriter, err := applog.New(*dataDir, os.Stderr, applog.Options{})
		if err != nil {
			return err
		}
		// The process owns this writer until exit so main can persist the final
		// command error after run returns.
		...
		configuration := config.Config{
			Listen: *listen, DataDir: *dataDir, Admin: admin, ConfigureLogging: logWriter.Configure,
		}
		for {
			...
			privateBackups, err := backupstore.New(configuration.DataDir)
			if err != nil {
				return err
			}
			created, err := privateBackups.ApplyPending(ctx, backup.DefaultMaxBytes)
			if err != nil {
				return fmt.Errorf("apply pending UI backup: %w", err)
			}
			if created {
				slog.Info("Processed queued UI backup", "data", configuration.DataDir)
			}
			err = app.Run(ctx, configuration)
			if errors.Is(err, app.ErrRestartReady) {
				admin, err = config.AdminCredentialsFromEnvironment()
				if err != nil {
					return err
				}
...
func runBackup(ctx context.Context, command string, args []string, output io.Writer) error {
	...
	var err error
	if command == "backup" {
		db, openErr := database.Open(ctx, *dataDir)
		if openErr != nil {
			return openErr
		}
