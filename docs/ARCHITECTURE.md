...
duration, the image referer allowlist, trusted proxy CIDRs, ordered HFS roots, the Bing automatic synchronization policy,
and local-log enablement, rotation size, and retention. Setup writes
...
Server logs are always written to stderr and are written by default to
`logs/kaven-media.log`. Administrators can disable local-file output and change
the rotation size (1 MiB–1 GiB) and retained-file count (0–100) in Settings;
zero disables automatic deletion of rotated files. The defaults are 10 MiB and
five older files. Rotated filenames contain the UTC rotation time so operators
can identify the covered period directly from a directory listing.
The log directory is private application state: media roots cannot overlap it,
HFS cannot expose it, and the restore directory picker omits it. Logs remain in
place across database-only backup/restore operations and are not included in a
database snapshot.
...
internal/applog          persistent console-mirrored log rotation
...
from their signature, and atomically published under `download/bing/<year>/`
using the readable, stable `<copyright>_<hsh>.<extension>` filename format.
Characters that are invalid in portable filenames are removed, and an overlong
copyright is UTF-8-safely truncated while the Bing hash suffix is preserved. The SQLite
...
records and recognizes the earlier 64-character URL-digest filenames created
by this application. It automatically renames them to the readable format
before updating SQLite. Renames never replace an existing file. If the
canonical readable name already exists, the migrated file receives a short
URL-digest qualifier before the Bing hash so both files remain available and
the final hash remains recoverable. A failed database update rolls the rename
back.
...
The UI supports English (`en-US`) and Simplified Chinese (`zh-CN`). On first
use it follows the browser language, then stores an explicit selection in
browser local storage. The selection also changes Quasar component labels and
locale-aware date and number formatting; it is not server configuration and is
...
The authenticated Activity page exposes bounded, newest-first views of image
access records and completed HFS download records. Its administrative API uses
explicit page and page-size parameters, caps each response at 100 records, and
returns the total count for server-side pagination. Records are read only in
the UI and include their stored request URL, source IP, resource identifier,
timestamp, and download User-Agent when present.

Source IP resolution uses the direct socket peer by default. When the peer is
inside an explicitly configured trusted proxy CIDR, the server walks
`X-Forwarded-For` from right to left through trusted hops and records the first
untrusted address. Invalid or oversized forwarding chains fall back to the
direct peer. This setting must contain only networks controlled by the operator.
...
Settings also displays the running build version and revision. Its authenticated
version endpoint returns local build identity without network access; only an
explicit update check contacts the fixed Kaven Media Server GitHub Releases API.
That request has a deadline and response-size limit, ignores drafts and invalid
release metadata, and compares valid semantic versions including RC identifiers.

Settings also offers a conservative repair attempt after an unhealthy check.
The UI first queues a validated database-only snapshot and waits for the
required application restart; the repair API rejects requests without a named,
valid private snapshot. For Bing files, repair requires a 32-character metadata
hash in the final filename component and validates both the image signature and
recent Bing metadata before reconstructing a row. For files below the configured
upload directory, it validates the image signature, calculates the size and
SHA-1, and either repairs a missing matching reference or creates a new UUID
record with explicitly reconstructed metadata. A checksum or Bing hash already
referenced by another usable file is reported as a duplicate together with the
existing database file reference and remains an orphan. Cache files, unmatched
metadata, and ambiguous cases remain untouched.
Repair never deletes, renames, or moves media files and always returns a fresh
integrity report. Reconstructed upload rows cannot restore an unknown legacy ID.
