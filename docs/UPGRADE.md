...
schema migrations and native libraries.

Reserve space for the database backup, separately preserved managed file trees,
a restored rehearsal copy, and subsequent uploads/cache growth. Capacity is not checked automatically before upgrades.
...
2. Run `check` and database `backup` with the installed version and the server's
   ...
   Preserve or replicate `upload/`, `cache/`, `download/bing/`, and HFS content separately.
3. Restore with that exact release into a new, absent directory. Use the
   [backup guide](BACKUP.md) for ownership, limits, and fresh-directory Docker
   commands. Format 3 snapshots reject a known application-version mismatch
   before copying; schema and integrity validation provide the remaining safety
   checks. Keep the original directory and backup untouched.
...
5. Place or mount the separately preserved managed trees, then start the candidate
   on the restored copy with the intended configuration,
...
the foreground; stop it before the final integrity check. Review HFS mappings
and external mounts before starting either version. Database snapshots are not
self-contained media backups; resolve all separately managed storage before
...
- Original URLs resolve by imported text ID, UUID, and SHA-1 with expected bytes;
...
- Image uploads remain public by default unless explicitly disabled; public HFS
  reads are exposed only where configured.
- After stopping the service, the candidate's `check` succeeds; restart and
  verify previously written data remains accessible.

A health response alone does not establish codec, schema, or storage correctness.
