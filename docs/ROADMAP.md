...
- [x] Implement `check` for database integrity and missing/orphaned files, with
      a read-only administrator UI action for the active server.
...
- [x] Implement lookup by imported text ID, UUID, and SHA-1.
...
- [x] Implement the protected image collection endpoint with a 100-row cap.
...
- [x] Implement an optional bounded domain allowlist with malformed-header and
      route coverage.
...
- [x] Make Bing scheduling configurable and expose a manual trigger in Settings.

## Phase 5: frontend and release

- [x] Integrate the owned Vue UI with the application API.
- [x] Fix same-origin API configuration and product branding in the owned
      `web/` frontend source.
...
- [x] Add offline database backup and verified restore commands/documentation.
- [x] Add an authenticated server-side backup restore UI with validated staging
      and an internal application restart for Docker deployments.
- [x] Add a private multi-snapshot `backup/` repository with restart-safe
      database-only creation and server-side restore that bypasses reverse-proxy
      body limits.
- [x] Add an authenticated, paginated Activity UI for image-access and HFS-download
      audit records.
- [x] Add persistent English and Simplified Chinese UI switching, including the
      one-time setup flow and Quasar component locale.
...
- [x] Reject unsupported recorded SQLite migration history before journal or
...
- [ ] Declare v1 after native container acceptance tests pass.
...
- Administrative UI for job status and history.
