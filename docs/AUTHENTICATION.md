...
clients from bypassing that proxy. Forwarded headers never establish a trusted
origin or scheme. `X-Forwarded-For` affects recorded client addresses only when
the direct peer belongs to an explicitly configured trusted proxy CIDR; it does
not alter the origin checks described above.
