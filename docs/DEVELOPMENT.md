...
VS Code users can select **Debug Kaven Media Server (API + UI dev)** and press F5. The checked-in
compound launch starts `cmd/kaven-media` on `127.0.0.1:5558` with the ignored
workspace `data/` directory and starts the Quasar development server on
`http://localhost:9000`. The browser opens the Quasar URL, whose proxy forwards
API requests to Go; Vue, TypeScript, and CSS changes are watched and hot-reloaded
without rebuilding or restarting the Go debugger. Stopping either launch stops
both. With an empty `data/` directory, F5 opens the first-run setup flow; the
administrator verifier is then retained in the ignored local SQLite database
for later debug sessions. The `webui` build tag remains reserved for embedded
...
An initialized administrator can run the same read-only check online from the
Storage integrity card in Settings. `POST /api/v1/admin/integrity/check`
returns the bounded JSON report and rejects a second concurrent check. Because
it streams original images to verify size and SHA-1, large libraries can take
time and consume storage I/O while the server remains available.

After an unhealthy check, Settings can queue a new validated private backup
and then call `POST /api/v1/admin/integrity/repair` with that backup name. The
server validates the snapshot before attempting repair, serializes repair with
integrity checks and Bing synchronization, and returns both a bounded repair
summary and a fresh integrity report. Repair remains conservative: it can
reconstruct recent Bing rows when an orphan filename supplies the matching
32-character Bing metadata hash, and it can rebuild uploaded-image rows from a
verified file signature, byte count, SHA-1, location, and modification time.
Rebuilt uploads receive a new UUID because an absent legacy ID cannot be
derived from file bytes. Files whose checksum or Bing hash already points to a
different usable file are reported separately as duplicates and remain orphaned.
The repair does not mutate media files or reconstruct cache rows.

Authenticated administrators can inspect audit data at `#/activity`. The page
calls `GET /api/v1/admin/activity?kind=access&page=1&pageSize=25` for image
accesses or uses `kind=download` for completed HFS downloads. Page size is
limited to 100 and results are ordered newest first. The endpoint is read only
and returns `Cache-Control: no-store`.

CLI backup and check require the server to be stopped. They share an OS lock with
`serve`. Backup and
...
Both operations require the SQLite bytes plus a 64 MiB reserve to be
...
lock release, interrupted-copy cleanup, database-only snapshots, and restoration
into a fresh directory with empty managed mount points.
...
Process-start settings use the `KAVEN_` prefix and CLI flags override their
environment equivalents. Application behavior is stored in SQLite and edited
from the first-run setup page or the authenticated Settings page.
...
| `KAVEN_ADMIN_SECURE_COOKIE` | none | `false` (direct HTTPS is detected automatically) |

`serve` always writes logs to stderr and writes them by default to
`<data-dir>/logs/kaven-media.log`. The Logging section in Settings can disable
local-file output and configure a 1 MiB–1 GiB rotation size and 0–100 retained
files; zero keeps all rotated files without automatic deletion, while defaults
are 10 MiB and five files. Docker needs no separate log volume
because the existing `/data` bind mount contains this directory. Database-only
snapshots deliberately exclude logs.

The active file is `kaven-media.log`. Rotation archives it with a UTC timestamp,
for example `kaven-media-20260916T133012.123456789Z.log`.

With no administrator environment credential, the compiled UI presents the
one-time setup page and persists its bcrypt verifier in SQLite. Environment
credentials override that stored verifier. This keeps the standard Docker start
zero-config while retaining secret-file support for managed deployments. The
setup-status response supplies runtime defaults to the page, including the
default relative HFS mapping for the configured upload directory.

Image uploads are public by default and can be disabled in Settings.
The database-backed application settings also contain the per-request file
count, maximum image and HFS file sizes, remembered-login duration, image
referer allowlist, trusted proxy CIDRs, ordered HFS root list, persistent logging policy, and Bing automatic synchronization
policy. Bing synchronization defaults to enabled every 24 hours; the interval
accepts 1-8,760 whole hours. Disabling it stops timer-driven downloads but does
not disable authenticated manual synchronization. Saving settings validates the
complete replacement and performs a brief internal server restart.

`uploadDirectory` defaults to `upload` and `downloadDirectory` defaults to
`download`. Both are portable, canonical paths relative to `<data-dir>`; they
must not be absolute, overlap each other, or use the reserved `cache`, `tmp`, or
`backup` trees. Bing files are written below `<downloadDirectory>/bing`.
Changing either setting creates the new destination on restart. Original-image
and Bing database references are relative to their configured roots, so neither
the root setting nor host paths such as `/data` or a Windows drive name are
repeated in each media row. Move the corresponding directory contents before
changing an established installation.

Adding `example.com` and `media.example.net` to the Settings domain list enables
referer filtering on original/derived image and metadata routes and Bing random
images, including HEAD requests. Specify up to 64 DNS names or IP literals.
Names are case-insensitive; schemes, ports,
...
Empty referers, localhost, loopback IPs, and private
...
No author-specific domains are built into a fresh installation.

The trusted proxy list is empty by default, so forwarded client-address headers
are ignored. Add at most 64 canonical IPv4 or IPv6 CIDRs, one per line in
Settings, only for reverse proxies under your control. For a Docker bridge this
is commonly a specific network such as `172.17.0.0/16`; confirm the actual
network with the deployment rather than copying that example blindly. A trusted
proxy must send `X-Forwarded-For`. The server walks multi-proxy chains from
right to left and stops at the first untrusted address. Direct requests and
malformed or over-32-address chains use the socket peer instead. Never configure
`0.0.0.0/0` or `::/0` on an application port reachable without the proxy.
...
The Settings HFS list has at most 64 entries. Names use
letters, digits, `.`, `_`, `@`, and `-`; every path must identify an existing,
non-overlapping server directory. Relative paths are canonical paths below the
data directory; absolute paths support directories outside it. `backup`, `tmp`,
and internal operation directories cannot be exposed. In Docker, bind-mount
each external directory at the exact absolute container path used by the
mapping before saving the setting.

The application creates only `<data-dir>/upload` as a default HFS root. Public means
unauthenticated reads. Read only independently removes the authenticated write
route for any mapping; it does not change path validation. If a configured
directory is missing on a later startup, that root is logged and left inactive
while the rest of the server starts. Restoring the mount requires an
application restart before the root becomes active again. Symlinked or
otherwise unsafe roots are never skipped as merely unavailable.

For example, a host directory mounted as
`/volume1/docker/ftp/files/pub:/hfs/pub:rw` uses `/hfs/pub` as its HFS `path`
and may set `readOnly` to either `false` or `true`. A mapping below `/data`, such
as `/data/upload`, needs no additional Docker mount beyond the persistent
`/data` binding. When `readOnly` is true, also mounting the directory with
Docker's `:ro` option provides defense in depth.
...
opened. Supported image, audio, video, PDF, and configured text types
...
database opener; backup/restore require matching known application versions and
then validate the snapshot schema and SQLite integrity. Managed files are
backed up and remapped separately.
...
Managed media paths passed to `internal/storage` are relative and platform-neutral.
...
HFS roots resolve to existing absolute paths after checking every component for
symlinks. Relative settings resolve below the data directory; absolute settings
can target another filesystem. For an external HFS upload, staged bytes are
copied to a reserved, non-routable `.kaven-upload-*` temporary file beside the
destination and then published without overwrite.
...
events. The direct socket peer is recorded unless it belongs to a configured
trusted proxy CIDR with a valid `X-Forwarded-For` chain.

HFS download records use a separate queue with the same 256-event capacity,
two-second SQLite write deadline, five-second shutdown drain, and the same
trusted-proxy IP policy. File paths and request targets are limited to 8 KiB, IP strings to
...
Downloaded bytes are staged, signature-validated, and published below
`download/bing/<year>/` as `<copyright>_<hsh>.<extension>` so HFS listings retain
the human-readable Bing description. Invalid portable filename characters are
removed and overlong descriptions are UTF-8-safely truncated; malformed dates use
`download/bing/unknown/`.
Before fetching current metadata, synchronization scans all downloaded Bing
database records and automatically renames this application's earlier
64-character URL-digest archive names to the readable format. It updates the
database without overwriting collisions; a short URL-digest qualifier preserves
both files, and a failed database update rolls the rename back.
Synchronization is safe to rerun and serializes concurrent calls. By default
the server runs it every 24 hours, beginning after the first configured interval
rather than during startup. The Settings page can disable automatic runs or select a 1-8,760 hour
interval. Scheduled invocations never overlap. Shutdown cancels active metadata
...
With administration enabled, `POST /api/v1/admin/bing/sync` enqueues archive
synchronization and returns an empty HTTP 202 response:

- `/api/v1/admin/bing/sync`

The routes are unavailable when administrative credentials are disabled.
Only one pending manual run is retained while synchronization is active.
The Settings page exposes the versioned route through its Synchronize now
button. No additional Docker volume or secret is required; downloads remain
under `/data/download/bing`.

## UI workflow

The application toolbar switches between English and Simplified Chinese. The
choice is persisted under the browser-local `kaven-media-locale` key; clear it
when testing first-visit locale detection. The one-time setup page provides the
same selector because it renders outside the normal application layout.

The Settings version section reads the embedded build identity from
`GET /api/v1/admin/version`. Adding `?check` performs the bounded outbound
GitHub Releases request used by the Check for updates button. Development builds
display `dev` and can show the latest release, but are not treated as an ordered
semantic version.
...
linting, and build details. Do not commit `node_modules`, generated assets,
or temporary frontend build caches.

## Commit discipline

Keep changes small enough to verify. A new endpoint should normally arrive
with its contract tests and documentation update in the same commit. Avoid
mixing mechanical restructuring with behavioral changes.
