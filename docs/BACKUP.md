...
Bing, HFS, logs, temporary, and private backup directories are never copied or
restored. This keeps snapshots small and lets operators manage large file trees
with normal host storage, mounts, replication, or other backup tools. Persistent
logs remain under `<data-dir>/logs` during an in-place Web restore; copy that
directory separately when moving to a new data directory.
...
The **Settings > Storage integrity** repair action always creates a new private
snapshot before it calls the repair API. Keep that snapshot until the resulting
integrity report has been reviewed. The snapshot protects database rows only;
the conservative repair currently does not delete, rename, or move media
files. It reconstructs unambiguous recent Bing records matched by the
32-character metadata hash in an orphan filename and uploaded-image records
whose signature, size, SHA-1, and location can be verified. A reconstructed
upload receives a new UUID; an unknown legacy ID cannot be recovered from the
file. Duplicate, unmatched, cache, and otherwise ambiguous files remain
