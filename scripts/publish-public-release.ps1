...
try {
    ...
    if ($actualParents.Count -ne 0) {
        throw 'Every public snapshot must be an orphan commit.'
    }
    ...
    if ($publicParent) {
        Write-Host "Replaces snapshot: $publicParent"
    } else {
        Write-Host 'Replaces snapshot: none (first release)'
    }
    ...
    if ($publicParent) {
        Invoke-Git -Arguments @(
            'push', '--atomic', "--force-with-lease=${branchReference}:${publicParent}", $Remote,
