...
function handleResponse(info: {
    ...
    if (info.xhr.status === 401) {
        ...
        for (const file of info.files) {
            file.status = "Administrator login required";
    ...
    if (info.xhr.status === 0 || !info.xhr.responseText.trim()) {
        for (const file of info.files) {
            file.status = info.xhr.status === 0 ? "Connection to server lost" : "Server returned an empty response";
            file.statusColor = "negative";
        }
        return;
    }
    ...
    try {
        ...
        for (const file of info.files) {
            file.status = `Invalid server response (${info.xhr.status || "network error"})`;
            file.statusColor = "negative";
        }
        return;
    }
