...
// RenameFile moves a regular file within the data directory without replacing
// an existing destination. Creating the destination hard link first makes the
// no-replacement guarantee atomic on the supported local filesystems.
func (store *Store) RenameFile(source, destination string) error {
	sourcePath, err := store.ResolveExisting(source)
	if err != nil {
		return fmt.Errorf("rename storage file %q: %w", source, err)
	}
	...
	if !info.Mode().IsRegular() {
		return fmt.Errorf("rename storage file %q: %w: not a regular file", source, ErrInvalidPath)
	}
	destinationPath, err := store.Resolve(destination)
	if err != nil {
		return fmt.Errorf("rename storage file to %q: %w", destination, err)
	}
	if err := os.Link(sourcePath, destinationPath); err != nil {
		if errors.Is(err, os.ErrExist) {
			return fmt.Errorf("rename storage file to %q: %w", destination, ErrExists)
		}
		return fmt.Errorf("rename storage file to %q: %w", destination, err)
	}
	if err := os.Remove(sourcePath); err != nil {
		cleanupErr := os.Remove(destinationPath)
		if cleanupErr != nil {
			return errors.Join(
				fmt.Errorf("rename storage file %q: remove source: %w", source, err),
				fmt.Errorf("rename storage file %q: roll back destination: %w", destination, cleanupErr),
			)
		}
		return fmt.Errorf("rename storage file %q: remove source: %w", source, err)
	}
	return nil
}
