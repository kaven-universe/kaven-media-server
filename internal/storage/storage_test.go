...
func TestResolveStoredFileReferenceAcceptsCurrentLayouts(t *testing.T) {
	store, err := New(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	paths := []struct{ kind, relative, stored string }{
		{"image", "2018/original.jpg", "upload/2018/original.jpg"},
		{"image_cache", "cache/ab/derived.jpg", "cache/ab/derived.jpg"},
		{"bing_image", "2018/wallpaper.jpg", "download/bing/2018/wallpaper.jpg"},
	}
	for _, item := range paths {
		if err := store.MkdirAll(path.Dir(item.stored), 0o755); err != nil {
			t.Fatal(err)
		}
		absolute := filepath.Join(store.Root(), filepath.FromSlash(item.stored))
		if err := os.WriteFile(absolute, []byte(item.kind), 0o600); err != nil {
			t.Fatal(err)
		}
		if got, err := store.ResolveStoredFileReference(item.relative, item.kind); err != nil || got != absolute {
			t.Errorf("%s reference = %q, %v", item.kind, got, err)
		}
	}
	for _, test := range []struct{ reference, kind string }{
		{"../escape.jpg", "image"},
		{"folder/../escape.jpg", "bing_image"},
		{`folder\file.jpg`, "image"},
		{"images/file.jpg", "unknown"},
	} {
		if _, err := store.ResolveStoredFileReference(test.reference, test.kind); !errors.Is(err, ErrInvalidPath) && !errors.Is(err, ErrPathEscape) {
			t.Errorf("reference %q kind %q error = %v", test.reference, test.kind, err)
		}
	}
}

func TestConfiguredMediaDirectoriesControlStoredReferences(t *testing.T) {
	store, err := New(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	...
	if store.UploadDirectory() != "media/uploads" || store.BingArchiveDirectory() != "archive/bing" {
		t.Fatalf("configured layout = %q, %q", store.UploadDirectory(), store.BingArchiveDirectory())
	}
	for _, item := range []struct{ kind, relative, stored string }{
		{"image", "2026/image.jpg", "media/uploads/2026/image.jpg"},
		{"bing_image", "2026/wallpaper.jpg", "archive/bing/2026/wallpaper.jpg"},
	} {
		if err := store.MkdirAll(path.Dir(item.stored), 0o755); err != nil {
			t.Fatal(err)
		}
		if err := os.WriteFile(filepath.Join(store.Root(), filepath.FromSlash(item.stored)), []byte("file"), 0o600); err != nil {
			t.Fatal(err)
		}
		if _, err := store.ResolveStoredFileReference(item.relative, item.kind); err != nil {
			t.Errorf("resolve %s: %v", item.relative, err)
		}
	}
	if err := store.MkdirAll("upload", 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(store.Root(), "upload", "old.jpg"), []byte("old"), 0o600); err != nil {
		t.Fatal(err)
	}
	if _, err := store.ResolveStoredFileReference("old.jpg", "image"); !errors.Is(err, ErrNotFound) {
		t.Fatalf("default upload directory remained active: %v", err)
	}
}
...
func TestRenameFileDoesNotReplaceDestination(t *testing.T) {
	store := newTestStore(t)
	if err := store.MkdirAll("images/2026", 0o755); err != nil {
		t.Fatal(err)
	}
	source := filepath.Join(store.Root(), "images", "2026", "opaque.jpg")
	destination := filepath.Join(store.Root(), "images", "2026", "readable.jpg")
	if err := os.WriteFile(source, []byte("source"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := store.RenameFile("images/2026/opaque.jpg", "images/2026/readable.jpg"); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(source); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("source still exists: %v", err)
	}
	if contents, err := os.ReadFile(destination); err != nil || string(contents) != "source" {
	...
	if err := store.RenameFile("images/2026/opaque.jpg", "images/2026/readable.jpg"); !errors.Is(err, ErrExists) {
		t.Fatalf("rename over destination error = %v, want ErrExists", err)
	}
	if contents, err := os.ReadFile(source); err != nil || string(contents) != "new source" {
		t.Fatalf("source changed after collision: %q, %v", contents, err)
	}
	if contents, err := os.ReadFile(destination); err != nil || string(contents) != "source" {
		t.Fatalf("destination changed after collision: %q, %v", contents, err)
	}
}
...
func TestStagedFileCommitsToAbsoluteDestinationWithoutOverwrite(t *testing.T) {
	store := newTestStore(t)
	destinationDirectory := t.TempDir()
	destination := filepath.Join(destinationDirectory, "published.txt")
	staged, err := store.Stage(context.Background(), strings.NewReader("published"), 100)
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = staged.Abort() })
	if result, err := staged.CommitAbsolute(destination, 0o640); err != nil || result != destination {
		t.Fatalf("commit result = %q, error = %v", result, err)
	}
	if contents, err := os.ReadFile(destination); err != nil || string(contents) != "published" {
		t.Fatalf("published contents = %q, error = %v", contents, err)
	}

	replacement, err := store.Stage(context.Background(), strings.NewReader("replacement"), 100)
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = replacement.Abort() })
	if _, err := replacement.CommitAbsolute(destination, 0o640); !errors.Is(err, ErrExists) {
		t.Fatalf("replacement error = %v, want ErrExists", err)
	}
	if contents, err := os.ReadFile(destination); err != nil || string(contents) != "published" {
		t.Fatalf("destination was replaced: %q, error = %v", contents, err)
	}
}
