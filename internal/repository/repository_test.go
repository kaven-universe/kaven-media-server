...
func TestImageRelationsRepositories(t *testing.T) {
	...
	total, err := accesses.Count(ctx)
	listed, listErr := accesses.List(ctx, 10, 0)
	if err != nil || listErr != nil || total != 1 || len(listed) != 1 || !reflect.DeepEqual(listed[0], access) {
		t.Fatalf("listed access records = %#v, total = %d, errors = %v, %v", listed, total, err, listErr)
	}
...
func TestBingImageRepository(t *testing.T) {
	...
	copyright, hash, file := "Example", "hash-1", "2026/example.jpg"
	...
	byHash, err := repository.GetByHash(ctx, hash)
	if err != nil || byHash.ID != image.ID {
		t.Fatalf("get Bing image by hash = %#v, error = %v", byHash, err)
	}
	...
	withFiles, err := repository.ListWithFiles(ctx)
	if err != nil || len(withFiles) != 1 || withFiles[0].ID != image.ID {
		t.Fatalf("downloaded Bing images = %#v, error = %v", withFiles, err)
	}
	...
	if _, err := repository.GetByHash(ctx, "missing"); !errors.Is(err, ErrNotFound) {
		t.Fatalf("missing Bing hash error = %v, want ErrNotFound", err)
	}
}
...
func TestDownloadRecordRepository(t *testing.T) {
	...
	listed, listErr := repository.List(ctx, 10, 0)
	if err != nil || listErr != nil || total != 1 || len(listed) != 1 || !reflect.DeepEqual(listed[0], record) {
		t.Fatalf("listed download records = %#v, total = %d, errors = %v, %v", listed, total, err, listErr)
	}
}
...
func testImage(suffix string, timestamp time.Time) Image {
	return Image{
		...
		Folder: "2026", Name: suffix + ".svg", OriginalName: suffix + ".svg",
