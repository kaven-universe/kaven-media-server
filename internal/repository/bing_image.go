...
func (repository *BingImageRepository) GetByHash(ctx context.Context, hash string) (BingImage, error) {
	image, err := scanBingImage(repository.db.QueryRowContext(ctx, bingImageSelect+" WHERE hsh = ?", hash))
	if err != nil {
		return BingImage{}, notFoundError("get Bing image by hash", err)
	}
	return image, nil
}
...
func (repository *BingImageRepository) ListWithFiles(ctx context.Context) ([]BingImage, error) {
	rows, err := repository.db.QueryContext(ctx, bingImageSelect+" WHERE file IS NOT NULL AND file <> '' ORDER BY id")
	if err != nil {
		return nil, fmt.Errorf("list downloaded Bing images: %w", err)
	}
	defer rows.Close()
	images := make([]BingImage, 0)
	for rows.Next() {
		image, err := scanBingImage(rows)
		if err != nil {
			return nil, err
		}
