...
func TestAdminSettingsRepositoryRoundTripsApplicationSettings(t *testing.T) {
	...
	if !settings.PublicUploads || settings.MaxFileCount != 100 || settings.RememberDurationDays != 30 ||
		...
		!settings.LocalLogEnabled || settings.LogMaxFileSize != config.DefaultLogMaxFileSize ||
		settings.LogMaxBackups != config.DefaultLogMaxBackups {
	...
	settings.TrustedProxyCIDRs = []string{"172.17.0.0/16"}
	settings.HFSRoots = []config.HFSRoot{{Name: "public", Path: "hfs/public", Public: true}}
	settings.LocalLogEnabled = false
	settings.LogMaxFileSize = 2 * 1024 * 1024
	settings.LogMaxBackups = 2
	...
	if stored.PublicUploads || stored.MaxFileCount != 12 || stored.BingSyncEnabled ||
		...
		stored.DownloadDirectory != "media/downloads" || len(stored.AllowedDomainNames) != 1 ||
		len(stored.TrustedProxyCIDRs) != 1 || stored.TrustedProxyCIDRs[0] != "172.17.0.0/16" || stored.HFSRoots[0].Name != "public" ||
		stored.LocalLogEnabled || stored.LogMaxFileSize != 2*1024*1024 || stored.LogMaxBackups != 2 {
