...
func (repository *AdminSettingsRepository) Get(ctx context.Context) (config.RuntimeSettings, error) {
	...
	var domainsJSON, proxiesJSON, rootsJSON string
	err := repository.db.QueryRowContext(ctx, `
		SELECT public_uploads, max_file_count, max_image_file_size, max_hfs_file_size,
		       ...
		       local_log_enabled, log_max_file_size, log_max_backups, trusted_proxy_cidrs
	...
	`).Scan(&settings.PublicUploads, &settings.MaxFileCount, &settings.MaxImageFileSize,
		...
		&settings.UploadDirectory, &settings.DownloadDirectory, &settings.LocalLogEnabled,
		&settings.LogMaxFileSize, &settings.LogMaxBackups, &proxiesJSON)
	if err != nil {
	...
	if err := json.Unmarshal([]byte(proxiesJSON), &settings.TrustedProxyCIDRs); err != nil {
		return config.RuntimeSettings{}, fmt.Errorf("decode trusted proxy CIDRs: %w", err)
	}
...
func (repository *AdminSettingsRepository) Update(ctx context.Context, settings config.RuntimeSettings) error {
	...
	proxiesJSON, err := json.Marshal(settings.TrustedProxyCIDRs)
	if err != nil {
		return fmt.Errorf("encode trusted proxy CIDRs: %w", err)
	}
	result, err := repository.db.ExecContext(ctx, `
		...
		upload_directory = ?, download_directory = ?, local_log_enabled = ?,
		log_max_file_size = ?, log_max_backups = ?, trusted_proxy_cidrs = ? WHERE id = 1
	`, settings.PublicUploads, settings.MaxFileCount, settings.MaxImageFileSize, settings.MaxHFSFileSize,
		...
		settings.LocalLogEnabled, settings.LogMaxFileSize, settings.LogMaxBackups, string(proxiesJSON))
	if err != nil {
