...
// RepairFileReference updates only the file-derived fields of an existing image
// record. It is used after a validated backup when the referenced file is gone
// but an orphan with the same checksum can safely replace that reference.
func (repository *ImageRepository) RepairFileReference(ctx context.Context, image Image) error {
	result, err := repository.db.ExecContext(ctx, `
		UPDATE images
		SET folder = ?, name = ?, original_name = ?, mime_type = ?, size = ?, updated_at = ?
		WHERE id = ?`,
		image.Folder, image.Name, image.OriginalName, image.MIMEType, image.Size,
		formatTimestamp(image.UpdatedAt), image.ID,
	)
	if err != nil {
		return writeError("repair image file reference", err)
	}
	count, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("repair image file reference: count affected rows: %w", err)
	}
	if count == 0 {
		return fmt.Errorf("repair image file reference: %w", ErrNotFound)
	}
	return nil
}
