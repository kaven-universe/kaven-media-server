...
const (
	...
	DefaultLocalLogEnabled       = true
	DefaultLogMaxFileSize        = int64(10 * 1024 * 1024)
	DefaultLogMaxBackups         = 5
	MaxLogMaxFileSize            = int64(1024 * 1024 * 1024)
	MaxLogMaxBackups             = 100
)
...
type Config struct {
	...
	ConfigureLogging      func(enabled bool, maxFileSize int64, maxBackups int) error
}
...
type RuntimeSettings struct {
	...
	LocalLogEnabled       bool      `json:"localLogEnabled"`
	LogMaxFileSize        int64     `json:"logMaxFileSize"`
...
func (configuration *Config) ApplyRuntimeSettings(settings RuntimeSettings) {
	...
	configuration.TrustedProxyCIDRs = settings.TrustedProxyCIDRs
...
func DefaultRuntimeSettings(dataDir string) RuntimeSettings {
	return RuntimeSettings{
		...
		AllowedDomainNames:    []string{}, TrustedProxyCIDRs: []string{}, HFSRoots: DefaultHFSRoots(),
		LocalLogEnabled: DefaultLocalLogEnabled, LogMaxFileSize: DefaultLogMaxFileSize,
...
func ValidateRuntimeSettings(settings RuntimeSettings) error {
	...
	if settings.TrustedProxyCIDRs == nil {
		return fmt.Errorf("trustedProxyCIDRs must be an array")
	}
	...
	if settings.LogMaxFileSize < 1024*1024 || settings.LogMaxFileSize > MaxLogMaxFileSize {
		return fmt.Errorf("logMaxFileSize must be between 1 MiB and 1 GiB")
	}
	if settings.LogMaxBackups < 0 || settings.LogMaxBackups > MaxLogMaxBackups {
		return fmt.Errorf("logMaxBackups must be between 0 and %d (0 keeps all rotated files)", MaxLogMaxBackups)
	}
	...
	if _, err := clientip.ParseTrustedPrefixes(settings.TrustedProxyCIDRs); err != nil {
		return err
	}
	return nil
}
...
func validateMediaDirectory(name, value string) error {
	...
	if first == "cache" || first == "tmp" || first == "backup" || first == "logs" || strings.HasPrefix(first, ".kaven-") {
