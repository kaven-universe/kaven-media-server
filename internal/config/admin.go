...
func LoadAdminCredentials(lookup EnvironmentLookup) (AdminCredentials, error) {
	...
	secureCookie := false
	if value, set := lookup("KAVEN_ADMIN_SECURE_COOKIE"); set && value != "" {
		parsed, err := strconv.ParseBool(value)
		if err != nil {
			return AdminCredentials{}, fmt.Errorf("parse KAVEN_ADMIN_SECURE_COOKIE as boolean: %w", err)
		}
		secureCookie = parsed
	}
	...
	if !directSet && !fileSet {
		...
		return AdminCredentials{Username: username, SecureCookie: secureCookie}, nil
	}
	...
	return AdminCredentials{Enabled: true, Username: username, Password: password, SecureCookie: secureCookie}, nil
}
...
func ValidateAdminUsername(username string) error {
	if len(username) < 1 || len(username) > 64 {
		return errors.New("administrator username must be 1-64 characters")
	}
	for _, character := range username {
		if !isAdminUsernameCharacter(character) {
			return errors.New("administrator username contains an invalid character")
		}
	}
	return nil
}
