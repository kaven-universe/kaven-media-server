...
func TestLoadAdminCredentialsSecureCookie(t *testing.T) {
	credentials, err := LoadAdminCredentials(environment(map[string]string{
		"KAVEN_ADMIN_PASSWORD":      "correct horse battery staple",
		"KAVEN_ADMIN_SECURE_COOKIE": "true",
	}))
	if err != nil {
		t.Fatal(err)
	}
	if !credentials.SecureCookie {
		t.Fatal("secure cookie setting was not loaded")
	}
	if _, err := LoadAdminCredentials(environment(map[string]string{"KAVEN_ADMIN_SECURE_COOKIE": "sometimes"})); err == nil {
		t.Fatal("invalid secure cookie setting succeeded")
	}
}
