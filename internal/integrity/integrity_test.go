...
func TestCheckHealthyStorage(t *testing.T) {
	...
	image := repository.Image{
		ID: "image-1", UUID: "uuid-1", SHA1: imageSHA1("original"), Folder: "2026", Name: "original.jpg",
	...
	writeCheckFile(t, dataDir, "upload/2026/original.jpg", "original")
	writeCheckFile(t, dataDir, "cache/derived.jpg", "derived")
	writeCheckFile(t, dataDir, "download/bing/wallpaper.jpg", "wallpaper")
	...
	if len(report.SchemaVersions) != 5 || report.SchemaVersions[0] != 1 || report.SchemaVersions[1] != 2 || report.SchemaVersions[2] != 3 || report.SchemaVersions[3] != 4 || report.SchemaVersions[4] != 5 {
...
func TestCheckAcceptsImageAtUploadRoot(t *testing.T) {
	ctx := context.Background()
	dataDir, db := prepareCheck(t)
	timestamp := checkTime()
	contents := "root image"
	image := repository.Image{
		ID: "root-image", UUID: "root-uuid", SHA1: imageSHA1(contents), Folder: "", Name: "root.jpg",
		OriginalName: "root.jpg", MIMEType: "image/jpeg", Size: int64(len(contents)), UploadDate: timestamp,
		UploadIP: "127.0.0.1", CreatedAt: timestamp, UpdatedAt: timestamp,
	}
	if err := repository.NewImageRepository(db).Create(ctx, image); err != nil {
		t.Fatalf("create image: %v", err)
	}
	writeCheckFile(t, dataDir, "upload/root.jpg", contents)

	report, err := Check(ctx, db, dataDir)
	if err != nil {
		t.Fatalf("check storage: %v", err)
	}
	if !report.Healthy() || report.ReferencesChecked != 1 || report.FilesChecked != 1 {
		t.Fatalf("root image reported issues: %#v", report)
	}
}

func TestCheckDoesNotInventoryHFSFiles(t *testing.T) {
	dataDir, db := prepareCheck(t)
	if err := os.MkdirAll(filepath.Join(dataDir, "hfs", "public"), 0o755); err != nil {
		t.Fatal(err)
	}
	writeCheckFile(t, dataDir, "hfs/public/not-in-database.txt", "hfs content")
	report, err := Check(context.Background(), db, dataDir)
	if err != nil {
		t.Fatal(err)
	}
	if !report.Healthy() || report.ReferencesChecked != 0 || report.FilesChecked != 0 {
		t.Fatalf("HFS file affected integrity report: %#v", report)
	}
}
...
func TestCheckReportsConsistencyProblems(t *testing.T) {
	...
	missing := repository.Image{
		ID: "missing-image", UUID: "missing-uuid", SHA1: strings.Repeat("a", 40), Folder: "2026", Name: "missing.jpg",
	...
	writeCheckFile(t, dataDir, "upload/orphan.jpg", "orphan")
...
func TestCheckReportsImageChecksumMismatch(t *testing.T) {
	...
	timestamp := checkTime()
	image := repository.Image{
		ID: "image-1", UUID: "uuid-1", SHA1: imageSHA1("expected"), Folder: "2026", Name: "original.jpg",
		OriginalName: "original.jpg", MIMEType: "image/jpeg", Size: 8, UploadDate: timestamp,
		UploadIP: "127.0.0.1", CreatedAt: timestamp, UpdatedAt: timestamp,
	}
	if err := repository.NewImageRepository(db).Create(ctx, image); err != nil {
	...
	writeCheckFile(t, dataDir, "upload/2026/original.jpg", "tampered")
	report, err := Check(ctx, db, dataDir)
	if err != nil {
		t.Fatal(err)
	}
	...
	found := false
	for _, issue := range report.Issues {
		if issue.Kind == IssueChecksumMismatch && issue.Path == "2026/original.jpg" {
			found = true
		}
	}
	if !found {
...
func imageSHA1(contents string) string {
	digest := sha1.Sum([]byte(contents)) // #nosec G401 -- matches the stored image checksum field.
