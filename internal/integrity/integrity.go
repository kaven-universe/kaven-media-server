...
import (
	"context"
	"crypto/sha1" // SHA-1 verifies the checksum stored with image records.
	"database/sql"
	"encoding/hex"
	"encoding/json"
	"errors"
...
func (report Report) WriteJSON(writer io.Writer) error {
	if err := json.NewEncoder(writer).Encode(report.jsonValue()); err != nil {
		return fmt.Errorf("write integrity JSON report: %w", err)
	}
	return nil
}

// MarshalJSON keeps integrity reports embedded in another response consistent
// with the standalone report produced by WriteJSON.
func (report Report) MarshalJSON() ([]byte, error) {
	return json.Marshal(report.jsonValue())
}

func (report Report) jsonValue() any {
	value := struct {
...
func Check(ctx context.Context, db *sql.DB, dataDir string) (Report, error) {
	...
	settings, err := repository.NewAdminSettingsRepository(db).Get(ctx)
	if err != nil {
		return Report{}, fmt.Errorf("read media directory settings: %w", err)
	}
	if err := store.ConfigureMediaDirectories(settings.UploadDirectory, settings.DownloadDirectory); err != nil {
		return Report{}, fmt.Errorf("configure media directories: %w", err)
	}
	checker := checker{
		db:         db,
		store:      store,
...
func (checker *checker) collectReferences(ctx context.Context) error {
	queries := []struct {
		...
		{kind: "image", referenceKind: "image", query: "SELECT id, CASE WHEN folder = '' THEN name ELSE folder || '/' || name END FROM images"},
		{kind: "image cache", referenceKind: "image_cache", query: "SELECT id, folder || '/' || name FROM image_cache"},
		{kind: "Bing image", referenceKind: "bing_image", query: "SELECT id, file FROM bing_images WHERE file IS NOT NULL AND file <> ''"},
	}
	for _, item := range queries {
		...
		for rows.Next() {
			...
			resolved, err := checker.store.ResolveStoredFileReference(relative, item.referenceKind)
			if err != nil {
...
func (checker *checker) checkImageChecksums(ctx context.Context) error {
	rows, err := checker.db.QueryContext(ctx, "SELECT id, CASE WHEN folder = '' THEN name ELSE folder || '/' || name END, COALESCE(sha1, ''), size FROM images")
	if err != nil {
	...
	for rows.Next() {
		...
		resolved, err := checker.store.ResolveStoredFileReference(relative, "image")
		if err != nil {
...
func checksumImage(ctx context.Context, filename string, limit int64) (string, int64, error) {
	...
	defer file.Close()
	digest := sha1.New() // #nosec G401 -- matches the stored image checksum field.
...
func (checker *checker) scanManagedFiles(ctx context.Context) error {
	for _, directory := range []string{checker.store.UploadDirectory(), "cache", checker.store.BingArchiveDirectory()} {
