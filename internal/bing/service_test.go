...
func TestServiceDownloadsPersistsAndReusesBingImage(t *testing.T) {
	...
	expectedFile := filepath.ToSlash(filepath.Join("2026", archiveKey(metadata.images[0].URL)+".png"))
...
func TestServiceAdoptsValidOrphanWithoutDownloading(t *testing.T) {
	...
	relative := filepath.ToSlash(filepath.Join("2026", archiveKey(metadata.images[0].URL)+".png"))
