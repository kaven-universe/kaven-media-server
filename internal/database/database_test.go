...
func TestOpenRejectsUnsupportedHistoryWithoutChangingDatabase(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if wantError == "" {
				wantError = "not supported by this executable"
			}
			if err == nil || !strings.Contains(err.Error(), wantError) {
				t.Fatalf("error = %v, want actionable unsupported-schema message", err)
			}
...
func TestInitialSettingsUseCurrentStorageLayout(t *testing.T) {
	dataDir := t.TempDir()
	...
	defer db.Close()
	var roots string
	if err := db.QueryRow("SELECT hfs_roots FROM admin_settings WHERE id = 1").Scan(&roots); err != nil {
		t.Fatal(err)
	}
	if roots != `[]` {
		t.Fatalf("default HFS roots = %s", roots)
	}
	var uploadDirectory, downloadDirectory string
	if err := db.QueryRow("SELECT upload_directory, download_directory FROM admin_settings WHERE id = 1").Scan(&uploadDirectory, &downloadDirectory); err != nil {
		t.Fatal(err)
	}
	if uploadDirectory != "upload" || downloadDirectory != "download" {
		t.Fatalf("media directories = upload %q, download %q", uploadDirectory, downloadDirectory)
	}
}
