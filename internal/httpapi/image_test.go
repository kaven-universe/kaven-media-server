...
func TestImageHandlerMetadataByEveryIdentifier(t *testing.T) {
	...
	image := repository.Image{
		...
		SHA1: "abcdef0123456789abcdef0123456789abcdef01", Folder: "2026/09",
	...
	for _, identifier := range []string{image.ID, image.UUID, image.SHA1} {
		t.Run(identifier, func(t *testing.T) {
			...
			if metadata.ID != image.ID || metadata.UUID != image.UUID || metadata.SHA1 != image.SHA1 || metadata.Path != "2026/09/stored.png" {
...
func TestImageHandlerServesOriginalWithHeadersAndRanges(t *testing.T) {
	...
	image := repository.Image{
		...
		Folder: "2026/09", Name: "stored.png", OriginalName: "original.png",
		MIMEType: "text/html",
	}
	fake := &imageLookupRepository{image: image}
	handler, store := newImageTestHandler(t, fake)
	storageFolder := path.Join(store.UploadDirectory(), image.Folder)
	if err := store.MkdirAll(storageFolder, 0o755); err != nil {
		t.Fatalf("create image folder: %v", err)
	}
	filePath := filepath.Join(store.Root(), filepath.FromSlash(storageFolder), image.Name)
...
func TestImageHandlerServesPreservedOriginalPath(t *testing.T) {
	contents := []byte("\x89PNG\r\n\x1a\npreserved-original")
	image := repository.Image{
		ID: "abcdef0123456789abcdef01", Folder: "201810/client",
		Name: "original.png", OriginalName: "original.png",
	}
	handler, store := newImageTestHandler(t, &imageLookupRepository{image: image})
	storageFolder := path.Join(store.UploadDirectory(), image.Folder)
	if err := store.MkdirAll(storageFolder, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(store.Root(), filepath.FromSlash(storageFolder), image.Name), contents, 0o600); err != nil {
		t.Fatal(err)
	}
	response := serveImageRequest(handler, "/image/"+image.ID)
	if response.Code != http.StatusOK || !bytes.Equal(response.Body.Bytes(), contents) {
...
func TestImageHandlerRejectsMissingUnsafeAndInvalidStoredFiles(t *testing.T) {
	tests := []struct {
		...
		{
			name: "missing", image: repository.Image{ID: "abcdef0123456789abcdef01", Folder: "2026", Name: "missing.png"},
		...
		{
			name: "invalid content", image: repository.Image{ID: "abcdef0123456789abcdef01", Folder: "2026", Name: "invalid.png", MIMEType: "image/png"},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			handler, store := newImageTestHandler(t, fake)
			if test.write != nil {
				storageFolder := path.Join(store.UploadDirectory(), test.image.Folder)
				if err := store.MkdirAll(storageFolder, 0o755); err != nil {
					t.Fatalf("create folder: %v", err)
				}
				if err := os.WriteFile(filepath.Join(store.Root(), filepath.FromSlash(storageFolder), test.image.Name), test.write, 0o600); err != nil {
...
func TestImageHandlerServesTransformation(t *testing.T) {
	image := repository.Image{ID: "abcdef0123456789abcdef01", Folder: "2026", Name: "source.jpg"}
	...
	if err := os.WriteFile(filepath.Join(store.Root(), filepath.FromSlash(storageFolder), image.Name), []byte("source"), 0o600); err != nil {
		t.Fatal(err)
	}
