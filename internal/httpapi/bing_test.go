...
func TestBingImageHandlerRetriesMissingFileAndServesImage(t *testing.T) {
	...
	handler, store, images := newBingTestHandler(t,
		bingResult{image: bingImageWithFile("missing.png")},
		bingResult{image: bingImageWithFile("2026/wallpaper.png")},
	)
	if err := store.MkdirAll("download/bing/2026", 0o755); err != nil {
		t.Fatal(err)
	}
	filePath := filepath.Join(store.Root(), "download", "bing", "2026", "wallpaper.png")
...
func TestBingImageHandlerServesRelativeArchivePath(t *testing.T) {
	contents := []byte("\x89PNG\r\n\x1a\npreserved-bing")
	reference := "2018/wallpaper.png"
	handler, store, _ := newBingTestHandler(t, bingResult{image: bingImageWithFile(reference)})
	if err := store.MkdirAll("download/bing/2018", 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(store.Root(), "download", "bing", filepath.FromSlash(reference)), contents, 0o600); err != nil {
		t.Fatal(err)
	}
	response := serveBingRequest(handler, http.MethodGet, nil)
	if response.Code != http.StatusOK || !bytes.Equal(response.Body.Bytes(), contents) {
...
func TestBingImageHandlerSupportsHeadRangesAndConditionalRequests(t *testing.T) {
	...
	handler, store, images := newBingTestHandler(t, bingResult{image: bingImageWithFile("image.jpg")})
	if err := store.MkdirAll("download/bing", 0o755); err != nil {
		t.Fatal(err)
	}
	filePath := filepath.Join(store.Root(), "download", "bing", "image.jpg")
	if err := os.WriteFile(filePath, contents, 0o600); err != nil {
		t.Fatal(err)
	}
	modified := time.Date(2026, time.September, 4, 1, 2, 3, 0, time.UTC)
...
func TestBingImageHandlerReturnsNotFoundAfterTwentyMissingFiles(t *testing.T) {
	...
	for index := range results {
		results[index].image = bingImageWithFile("missing.jpg")
	}
...
func TestBingImageHandlerRejectsUnsafeAndInvalidStoredFiles(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "path traversal", file: "../image.png", status: http.StatusInternalServerError},
		{name: "invalid bytes", file: "invalid.png", contents: []byte("not an image"), status: http.StatusUnsupportedMediaType},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.contents != nil {
				...
				if err := os.WriteFile(filepath.Join(store.Root(), "download", "bing", "invalid.png"), test.contents, 0o600); err != nil {
					t.Fatal(err)
				}
			}
			response := serveBingRequest(handler, http.MethodGet, nil)
...
func TestBingImageHandlerRejectsSymlink(t *testing.T) {
	handler, store, _ := newBingTestHandler(t, bingResult{image: bingImageWithFile("link.jpg")})
	if err := store.MkdirAll("download/bing", 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.Symlink(target, filepath.Join(store.Root(), "download", "bing", "link.jpg")); err != nil {
