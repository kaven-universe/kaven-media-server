...
func (handler *BingImageHandler) open(image repository.BingImage) (*os.File, os.FileInfo, imageproc.Format, error) {
	...
	resolved, err := handler.store.ResolveStoredFileReference(reference, "bing_image")
	if err != nil {
	...
	verifiedPath, err := handler.store.ResolveStoredFileReference(reference, "bing_image")
	if err != nil {
