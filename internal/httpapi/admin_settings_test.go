...
func TestApplicationSettingsHandlerValidatesAndRestarts(t *testing.T) {
	...
	body := `{"uploadDirectory":"media/uploads","downloadDirectory":"media/downloads","publicUploads":false,"maxFileCount":25,"maxImageFileSize":10485760,"maxHFSFileSize":20971520,"rememberDurationDays":60,"bingSyncEnabled":false,"bingSyncIntervalHours":12,"allowedDomainNames":["example.com"],"trustedProxyCIDRs":["172.17.0.0/16"],"hfsRoots":[{"name":"files","path":"hfs/files","public":true}],"localLogEnabled":true,"logMaxFileSize":2097152,"logMaxBackups":3}`
	...
	if response.Code != http.StatusOK || store.updates != 1 || restarts != 1 || store.settings.PublicUploads ||
		...
		len(store.settings.TrustedProxyCIDRs) != 1 || store.settings.TrustedProxyCIDRs[0] != "172.17.0.0/16" ||
		!store.settings.LocalLogEnabled || store.settings.LogMaxFileSize != 2097152 || store.settings.LogMaxBackups != 3 {
