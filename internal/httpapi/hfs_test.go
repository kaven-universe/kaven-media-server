...
func TestHFSRootHandlerReturnsEntriesInConfigurationOrder(t *testing.T) {
	store, err := storage.New(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	registry, err := hfs.NewRegistry(store, []config.HFSRoot{
		{Name: "uploaded", Path: createHTTPHFSRoot(t, store, "hfs/uploaded")},
		{Name: "shared", Path: createHTTPHFSRoot(t, store, "hfs/shared"), Public: true},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestHFSPathHandlerRendersSafeTypesAndDownloadsOthers(t *testing.T) {
	store, registry, root := testHFSPathHandler(t, true)
	text := []byte("Kaven Media Server HFS fixture.\n")
...
func TestHFSPathHandlerRecordsOnlyCompletedDownloads(t *testing.T) {
	...
	if len(recorder.records) != 1 || recorder.records[0] != (hfsDownloadRecord{
		file: filepath.Join(root.Path, "file.bin"), originalURL: "/hfs/shared/file.bin?download",
...
func TestHFSWriteHandlerCreatesDirectory(t *testing.T) {
...
func testHFSPathHandler(t *testing.T, public bool) (*storage.Store, *hfs.Registry, hfs.Root) {
	...
	registry, err := hfs.NewRegistry(store, []config.HFSRoot{{Name: "shared", Path: createHTTPHFSRoot(t, store, "hfs/shared"), Public: public}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestHFSRootHandlerOmitsRemovedRoot(t *testing.T) {
	...
	registry, err := hfs.NewRegistry(store, []config.HFSRoot{{Name: "removed", Path: createHTTPHFSRoot(t, store, "hfs/removed")}})
	if err != nil {
		t.Fatal(err)
	}
...
func createHTTPHFSRoot(t *testing.T, store *storage.Store, relative string) string {
	t.Helper()
