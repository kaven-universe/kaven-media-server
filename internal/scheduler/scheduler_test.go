...
func TestDisabledScheduleStillAcceptsManualTrigger(t *testing.T) {
	started := make(chan struct{}, 1)
	scheduled, err := New(Job{
		Name: "archive", Interval: time.Hour, Disabled: true,
		Run: func(context.Context) error {
			started <- struct{}{}
			return nil
		},
	...
	scheduled.newTicker = func(time.Duration) ticker {
		t.Fatal("disabled schedule created a ticker")
		return nil
	}
	ctx, cancel := context.WithCancel(context.Background())
	done := make(chan error, 1)
	go func() { done <- scheduled.Run(ctx) }()
	if queued, err := scheduled.Trigger("archive"); err != nil || !queued {
		t.Fatalf("manual trigger = %v, %v", queued, err)
	}
	awaitSignal(t, started, "manual invocation")
	cancel()
	if err := awaitResult(t, done, "scheduler shutdown"); err != nil {
		t.Fatal(err)
	}
