...
func validateDatabase(ctx context.Context, directory string) error {
	...
	references := 0
	for _, query := range []string{"SELECT CASE WHEN folder = '' THEN name ELSE folder || '/' || name END FROM images", "SELECT folder || '/' || name FROM image_cache", "SELECT file FROM bing_images WHERE file IS NOT NULL AND file <> ''"} {
