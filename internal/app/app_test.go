...
func TestUIRestoreRouteRequiresConfiguredAdministrator(t *testing.T) {
	...
	if response := serve(t, disabled, http.MethodGet, "/api/v1/admin/activity"); response.Code != http.StatusNotFound {
		t.Fatalf("disabled activity status = %d, want 404", response.Code)
	}
	...
	assertRequiresAdmin(t, serve(t, protected, http.MethodGet, "/api/v1/admin/activity"))
...
func TestHFSDownloadUsesForwardedAddressFromTrustedProxy(t *testing.T) {
	dataDir := t.TempDir()
	db, err := database.Open(context.Background(), dataDir)
	if err != nil {
		t.Fatal(err)
	...
	configuration := config.Config{
		...
		HFSRoots:          []config.HFSRoot{{Name: "public", Path: createAppHFSRoot(t, store, "hfs/public"), Public: true}},
	}
	handler, err := routes(db, configuration, store, processor, accessRecorder, downloadRecorder, nil)
	if err != nil {
		t.Fatal(err)
	...
	request := httptest.NewRequest(http.MethodGet, "/hfs/public/archive.bin?download", nil)
	request.RemoteAddr = "172.17.0.1:1234"
	...
	var ip string
	if err := db.QueryRowContext(context.Background(), "SELECT ip FROM download_records").Scan(&ip); err != nil {
		t.Fatal(err)
	}
	if ip != "198.51.100.7" {
		...
		if err == nil && migrations == 5 {
			...
			if err == nil {
				...
				if json.Unmarshal([]byte(rootsJSON), &roots) == nil && len(roots) == 0 {
					if _, statErr := os.Stat(filepath.Join(dataDir, "download", "bing")); statErr == nil {
						break
					}
				}
			}
