...
func (service *Service) Save(ctx context.Context, input Input) (Result, error) {
	...
	now := service.now().UTC()
	folder := path.Join(now.Format("2006"), now.Format("01"))
	storageFolder := path.Join(service.store.UploadDirectory(), folder)
	...
	storageRelative := path.Join(service.store.UploadDirectory(), relative)
	if _, err := input.File.Commit(storageRelative, 0o640); err != nil {
	...
	if err := service.images.Create(ctx, image); err != nil {
		if removeErr := service.store.RemoveFile(storageRelative); removeErr != nil {
