...
func TestSaveImageAndDetectDuplicate(t *testing.T) {
	...
	if err := store.ConfigureMediaDirectories("media/uploads", "media/downloads"); err != nil {
		t.Fatal(err)
	}
	...
	if stored.ID != result.ID || stored.Folder != "2026/09" || stored.Name != result.UUID+".png" || stored.MIMEType != "image/png" || stored.OriginalName != "display.png" {
		t.Fatalf("stored image = %#v", stored)
	}
	file, err := os.ReadFile(filepath.Join(store.Root(), filepath.FromSlash(store.UploadDirectory()), filepath.FromSlash(stored.Folder), stored.Name))
...
func TestSaveRollsBackPublishedFileWhenDatabaseWriteFails(t *testing.T) {
	...
	entries, err := os.ReadDir(filepath.Join(store.Root(), "upload", "2026", "09"))
	if err != nil {
