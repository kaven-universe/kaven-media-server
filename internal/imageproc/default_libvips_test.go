...
func TestLibvipsProcessorCanBeRecreated(t *testing.T) {
	first, err := NewDefaultProcessor(1)
	if err != nil {
		t.Fatal(err)
	}
	first.Close()

	second, err := NewDefaultProcessor(1)
	if err != nil {
		t.Fatalf("recreate processor after close: %v", err)
	}
	second.Close()
}
