...
func NewDefaultProcessor(maxConcurrent int) (*Processor, error) {
	...
	if !defaultRuntime.started {
		if err := vips.Startup(nil); err != nil {
			return nil, fmt.Errorf("start libvips: %w", err)
		}
		defaultRuntime.started = true
	}
	return NewProcessor(vipsBackend{}, maxConcurrent)
}

// ShutdownDefaultRuntime releases the process-wide libvips runtime. Govips
// cannot restart libvips after shutdown, so individual processors must not own
// its lifetime: the serve command can recreate them during an internal restart.
func ShutdownDefaultRuntime() {
	defaultRuntime.Lock()
	defer defaultRuntime.Unlock()
	if !defaultRuntime.started {
		return
